...
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
	...
	<title><%= request.getParameter("Pizza") %></title>
	<!-- Bootstrap Core CSS -->
...
<div class="brand">Zee Pizza</div>
    ...
    <div class="container">
    	<div class="row">
            <div class="box">
            <h3>Choose Size</h3>
            <!-- gets the sizes for pizzas -->
                <sql:query dataSource="${ds}" var="rs">SELECT ProductID, ProductName, PizzaSize, Price from Product where ProductName = '<%= request.getParameter("Pizza") %>'</sql:query>
                
				<select name="Pizza1" id="Pizza1" size="3">
				<c:forEach var = "row" items="${rs.rows}">
				<option value ="${row.ProductID}">${row.ProductName}   ${row.PizzaSize}   $${row.Price}</option>
